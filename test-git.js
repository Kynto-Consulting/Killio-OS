    ...
    try {
        ...
        const gitFs = {
            promises: {
                ...
                unlink: async (filepath) => { console.log('unlink', filepath); await kernel.getVFS().deleteNode(filepath); },
                ...
                rmdir: async (filepath) => { console.log('rmdir', filepath); await kernel.getVFS().deleteNode(filepath); },
                stat: async (filepath) => {
                    const node = await kernel.getVFS().getNode(filepath);
                    if (!node) {
                ...
                lstat: async (filepath) => gitFs.promises.stat(filepath),
