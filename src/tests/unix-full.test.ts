...
async function runTest() {
  ...
  const testSteps = [
    ...
    { cmd: ['write_file', 'notes.txt', 'Hello Killio OS!'], expected: (out: string) => out.includes('Updated file') || out.includes('Wrote to file') },
    { cmd: ['cat', 'notes.txt'], expected: 'Hello Killio OS!' },
    { cmd: ['ls', '-l'], expected: (out: string) => out.includes('notes.txt') && out.includes('todo.md') },
    ...
    { cmd: ['ls'], expected: (out: string) => !out.includes('old_notes.txt') },
    { cmd: ['cd', '/'], expected: 'Changed directory to /' },
    { cmd: ['rm', '-rf', '/home/agent/docs'], expected: '' },
    { cmd: ['ls', '/home/agent'], expected: (out: string) => !out.includes('docs') },
  ];
