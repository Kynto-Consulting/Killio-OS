...
function renderOMP(configStr: string, kernel: KillioKernel, gitBranch: string) {
  try {
    ...
    const parseInlineColors = (text: string) => {
      let output = '';
      let currentIdx = 0;
    ...
    for (const block of blocks) {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        if (seg.type === 'git' && !gitBranch) continue;
...
function getPrompt(kernel: KillioKernel, gitBranch: string = '') {
  const cwd = kernel.getCWD();
  const user = kernel.getCurrentUser();
  const hostname = kernel.getHostname();
  const displayCwd = cwd === `/home/${user}` || (user === 'root' && cwd === '/root') ? '~' : cwd;
  ...
  for (const cond of conditionals) {
    ...
    let tokenValue = '';
    if (tokenName === 'git_branch') tokenValue = gitBranch;
...
async function fetchGitBranch(kernel: KillioKernel): Promise<string> {
  try {
...
function renderNano(path: string, buffer: string[], modified: boolean, cursorLine: number, cursorChar: number, statusMsg?: string) {
...
async function startCLI() {
  ...
  const rl = readline.createInterface({
    ...
    completer: (line: string, callback: (err: any, result: [string[], string]) => void) => {
      ...
      if (parts.length <= 1) {
        const commands = Array.from((kernel as any).commands.keys()) as string[];
          ...
          const handleKey = async (str: string, key: any) => {
            ...
            if (key.ctrl) {
              ...
              if (key.name === 'up') {
                ...
                if (cursorChar < (buffer[cursorLine]?.length || 0)) cursorChar++;
