 ...
 */
async function runInIsolatedVm(code, kernel, cwd, opts = {}) {
    let ivm;
    try {
    ...
    try {
        ...
        await jail.set('__log', new ivm.Reference((s) => { out.push(String(s)); }));
        await jail.set('__fs', new ivm.Reference(async (op, a, b) => {
            const p = a.startsWith('/') ? a : cwd.replace(/\/$/, '') + '/' + a;
            switch (op) {
                case 'writeFile':
                    await kernel.writeFile(p, b ?? '');
                    return '';
                case 'readFile': return await kernel.readFile(p);
                case 'exists': return (await kernel.getVFS().getNode(kernel.resolvePath(p))) ? '1' : '';
                case 'mkdir':
                    await kernel.execute(['mkdir', '-p', p]);
                    return '';
                case 'unlink':
                    await kernel.execute(['rm', p]);
                    return '';
 ...
 */
async function runInJsSandbox(body, api, timeoutMs = 5000) {
...
export const node = async (args, kernel) => {
    ...
    if (args[0] === '-e' && args[1]) {
        try {
            ...
            if (process.env.KILLIO_JS_ISOLATION === 'strict') {
                ...
                if (strict)
                    return { output: strict.output, exitCode: 0 };
            }
            ...
            const result = await runInJsSandbox(transpiledCode, {
                console: safeConsole,
                require: (p) => mockRequire(p, kernel.getCWD()),
    ...
    try {
        ...
        await runInJsSandbox(transpiledCode, {
            console: safeConsole,
            require: (p) => mockRequire(p, __dirname),
            module: moduleEnv,
