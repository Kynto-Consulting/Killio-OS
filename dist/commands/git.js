...
export const git = async (args, kernel) => {
    if (args.length === 0) {
    ...
    const gitFs = {
        promises: {
            ...
            lstat: async (filepath, options) => {
                return gitFs.promises.stat(filepath, options);
            },
    ...
    try {
        switch (subcommand) {
            ...
            case 'status': {
                ...
                const staged = status.filter(row => row[2] === 2 && row[3] === 2 && row[1] !== row[2]).map(row => row[0]);
            ...
            case 'checkout': {
                if (args.includes('-b')) {
                    ...
                    if (!newBranch)
                        return { output: 'fatal: branch name required for -b', exitCode: 1 };
                ...
                if (dd !== -1) {
                    ...
                    if (files.length === 0)
                        return { output: 'fatal: no paths given to restore', exitCode: 1 };
                ...
                const ref = args[1];
                if (!ref)
                    return { output: 'fatal: branch name or reference required', exitCode: 1 };
            ...
            case 'restore': {
                ...
                if (files.length === 1 && (files[0] === '.' || files[0] === kernel.getCWD())) {
                    ...
                    filepaths = (await gitApi.statusMatrix({ fs: gitFs, dir })).map(row => row[0]);
                }
                else {
            ...
            case 'add': {
                ...
                if (file === '.') {
                    ...
                    for (const row of status) {
                        if (row[2] !== row[3])
                            await gitApi.add({ fs: gitFs, dir, filepath: row[0] });
                    }
                }
                else {
            ...
            case 'remote': {
                ...
                if (op === 'add') {
                    ...
                    const url = args[3];
                    if (!name || !url)
                        return { output: 'usage: git remote add <name> <url>', exitCode: 1 };
        ...
        if (e.message.includes('not a git command'))
