 ...
 */
export const find = async (args, kernel) => {
    let start = '.';
    let typeFilter = null;
    let nameGlob = null;
    ...
    const lines = [];
    const emit = (path, type) => {
        if (typeFilter === 'f' && type !== 'file')
            return;
        if (typeFilter === 'd' && type !== 'directory')
            return;
        if (nameGlob) {
            const base = path.split('/').pop() || path;
            if (!nameGlob.test(base))
                return;
        }
        lines.push(path);
    };
    ...
    if (!startNode)
        return { output: `find: '${start}': No such file or directory`, exitCode: 1 };
    ...
    const walk = async (dir, depth) => {
        ...
        let children;
        try {
            children = await kernel.listNodes(dir);
        }
        catch {
            return;
        }
...
export const cat = async (args, kernel, stdin) => {
    ...
    if (files.length === 0) {
        ...
        for (const file of files) {
            ...
            try {
                bodies.push(await kernel.readFile(file));
            }
            catch (e) {
                bodies.push(`\0ERR${e.message}`);
                err = true;
            }
        }
    }
...
export const head = async (args, kernel, stdin) => {
    ...
    try {
        ...
        if (bytes)
            return { output: body.slice(0, n), exitCode: 0 };
        return { output: body.split('\n').slice(0, n).join('\n'), exitCode: 0 };
    }
    catch (e) {
        return { output: `head: ${e.message}`, exitCode: 1 };
    }
};
export const tail = async (args, kernel, stdin) => {
    const { n, bytes, files } = parseHeadTail(args);
    try {
        let body;
        if (files.length) {
            const node = await kernel.getVFS().getNode(kernel.resolvePath(files[0]));
            if (!node || node.type !== 'file')
                return { output: `tail: ${files[0]}: No such file or directory`, exitCode: 1 };
            body = node.content || '';
        }
        else
            body = stdin ?? '';
        if (bytes)
            return { output: body.slice(-n), exitCode: 0 };
        ...
        return { output: `tail: ${e.message}`, exitCode: 1 };
    }
};
