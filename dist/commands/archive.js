...
export const zip = async (args, kernel) => {
    if (args.length < 2)
        return { output: 'zip: usage: zip <archive.zip> <file1> <file2> ...', exitCode: 1 };
    ...
    try {
        ...
        if (added === 0)
            return { output: 'zip error: Nothing to do!', exitCode: 1 };
...
export const unzip = async (args, kernel) => {
    if (args.length < 1)
        return { output: 'unzip: usage: unzip <archive.zip>', exitCode: 1 };
    try {
