...
export class KillioKernel {
    ...
    async ensureDir(path, owner = 'root', permissions = '755') {
    ...
    async seedMockFile(path, content) {
        try {
    ...
    async readFile(path) {
        ...
        if (!this.checkPermission(node, 'r'))
    ...
    async writeFile(path, content, options = {}) {
        ...
        const node = await this.vfs.getNode(fullPath);
        if (node) {
            if (!this.checkPermission(node, 'w'))
                throw new Error(`EACCES: permission denied, open '${path}'`);
            ...
            if (parentNode && !this.checkPermission(parentNode, 'w'))
                throw new Error(`EACCES: permission denied, open '${path}'`);
            await this.vfs.createNode({
                path: fullPath,
                type: 'file',
        ...
        if (recursive) {
            ...
            for (const part of parts) {
                ...
                if (!existing) {
                    ...
                    const parentNode = await this.vfs.getNode(parentPath);
                    if (parentNode && !this.checkPermission(parentNode, 'w'))
                        throw new Error(`EACCES: permission denied, mkdir '${current}'`);
                    await this.vfs.createNode({
                        path: current,
                        type: 'directory',
                        ownerId: this.vfs.getOwnerId(),
    ...
    async listNodes(path) {
        const fullPath = this.resolvePath(path);
        const node = await this.vfs.getNode(fullPath);
        if (!node)
            throw new Error(`ENOENT: no such file or directory, ls '${path}'`);
        if (!this.checkPermission(node, 'r'))
            throw new Error(`EACCES: permission denied, ls '${path}'`);
    ...
    async chmod(path, mode) {
        ...
        if (!this.checkPermission(node, 'w'))
            throw new Error(`EACCES: permission denied, chmod '${path}'`);
    ...
    async chown(path, owner, group) {
        ...
        if (!this.checkPermission(node, 'w'))
            throw new Error(`EACCES: permission denied, chown '${path}'`);
        await this.vfs.updateNode(fullPath, { metadata: { ...node.metadata, owner, group } });
    }
    checkPermission(node, access) {
        if (!node)
            return true; // if node doesn't exist, permission implies parent check later
        ...
        if (access === 'r')
            return (targetDigit & 4) !== 0;
        if (access === 'w')
            return (targetDigit & 2) !== 0;
        if (access === 'x')
            return (targetDigit & 1) !== 0;
        return false;
    }
    expandVars(text) {
    ...
    async execute(command) {
        ...
        commands.push(current);
        let lastResult = { output: '', exitCode: 0 };
        let finalOutput = '';
        for (let i = 0; i < commands.length; i++) {
            ...
            if (sep === '&') {
                ...
                (async () => {
                    ...
                    for (const st of stages) {
                        if (!st.length)
                            continue;
                        const r = await this.runStage(st, pin);
                        pin = r.output;
                    }
            ...
            if (finalOutput && lastResult.output)
                finalOutput += '\n';
            finalOutput += lastResult.output;
            if (sep === '&')
                continue; // Don't block on background
        }
        return {
    ...
    async executeCapability(node, args) {
        ...
        if (type === 'js') {
            try {
                ...
                if (oldCmdName)
                    this.env.set('CMD_NAME', oldCmdName);
                else
                    this.env.delete('CMD_NAME');
                return result;
            }
            catch (e) {
        ...
        if (type === 'wasm') {
            try {
                ...
                const importObject = {
                    env: {
                        ...
                        vfs_get_size: (pathPtr, pathLen) => {
                            ...
                            const node = this.vfs.nodes?.get ? this.vfs.nodes.get(resolved) : null;
                            if (node && node.type === 'file') {
                                size = node.content?.length || 0;
                            }
                            return size;
                        },
                        vfs_read: (pathPtr, pathLen, bufPtr) => {
                            const path = this.readStringFromMemory(memory, pathPtr, pathLen);
                            const resolved = this.resolvePath(path);
                            const node = this.vfs.nodes?.get ? this.vfs.nodes.get(resolved) : null;
                            if (node && node.type === 'file') {
                        ...
                        vfs_write: (pathPtr, pathLen, contentPtr, contentLen) => {
                            const path = this.readStringFromMemory(memory, pathPtr, pathLen);
                ...
                const { instance } = await globalThis.WebAssembly.instantiate(wasmBuffer, importObject);
    ...
    readStringFromMemory(memory, ptr, len) {
        const bytes = new Uint8Array(memory.buffer, ptr, len);
        return new TextDecoder().decode(bytes);
    }
    async executeScript(node, args) {
    ...
    async seedInternalCapabilities() {
